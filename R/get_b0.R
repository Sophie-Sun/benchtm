...
get_b0 <- function(X, scal, prog, pred, trt, b1, type, 
  ...
  opt <- uniroot(function(b0, ...){
    calc_power(c(b0, b1), ...)[1] - power
  }, scal = scal, prog_vals=prog_vals, trt=trt, pred_vals=pred_vals, alpha = alpha, 
  type = type, sign_better = sign_better, sigma_error = sigma_error, interval = interval)
  out <- opt$root
  outpow <- calc_power(c(out,b1), scal=scal, prog_vals=prog_vals, trt=trt, pred_vals=pred_vals,
                       alpha = alpha, type = type,
                       sign_better = sign_better, sigma_error = sigma_error)[1]
