...
#' @param include_truth boolean, will the true prognostic and
#'   predictive effects be included in the outcome data-set?
#' @param sign_better whether larger response is better (used to
...
generate_y <- function(X, trt, prog, pred, b0, b1 = NULL, sd_te = NULL,
                       ...
                       sigma_error = 1.0, theta = 1.0, cens_time = NULL,
                       lambda0 = NULL,
                       include_truth = FALSE,
                       sign_better = 1) {
  ...
  if (type == "survival") {
    stopifnot(!is.null(cens_time) & !is.null(lambda0))
    lambda <- lambda0 * exp(y_link)
    dat$Y <- stats::rexp(n, lambda)
    c_time <- cens_time(n)
    dat$Y <- pmin(dat$Y, c_time) ## censor when c_time < event time
    dat$event <- as.numeric(dat$Y < c_time)
    form_tmp <- paste0(
      "log(lambda) = ", prog, " + Trt*", "(", round(b0, 4),
      "+", round(b1, 4), "*(", pred, "))"
    )
  }
